...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn(
      'border-b border-solid border border-neutral-200 bg-slate-100 dark:bg-neutral-900 data-[state=open]:border-blue-500 dark:border-neutral-600 px-1 py-0 mb-4',
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Header className="accordion-item__header flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        'accordion-item__trigger flex flex-1 items-center h-[36px] pb-[0.5rem] justify-between font-bold transition-all hover:underline [&[data-state=open]>svg]:rotate-180 focus:ring-0 focus:outline-none focus:border-none focus:text-blue-500',
        className,
      )}
      {...props}
    >
