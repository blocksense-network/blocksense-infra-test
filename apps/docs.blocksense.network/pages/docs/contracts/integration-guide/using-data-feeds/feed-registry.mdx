...
<Callout type="info" emoji="💡">
  The Feed Registry exclusively includes feeds associated with canonical token addresses on a given network. Feeds without a corresponding token address,
  such as those for stock indices, are not included in the registry due to the lack of a canonical token representation.
</Callout>
