...
Historic Data Feed Store contract is where all data is stored. It is written in Solidity assembly and contains only one function: `fallback`.
...
<Callout type="info">
  Use [ProxyCall library](https://github.com/blocksense-network/blocksense/tree/main/libs/contracts/contracts/libraries/ProxyCall.sol) for easier interaction with the contract.
</Callout>

In order to read data from the contract, the client must call the `fallback` function as follows:
...
```js copy
const historicDataFeedStore = new ethers.Contract(contractAddress, abiJson, provider);
