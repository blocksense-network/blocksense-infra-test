...
PLONKY2 is a zkSNARK built by [Polygon Labs](https://polygon.technology/), with efficiency, decomposition and size in mind. A proof can be generated 100x faster than other systems, proofs can be split into subproofs and distributed across hundreds or thousands of machines, and it provides the ability to shrink proofs down to \<50kb in seconds.
...
3. Update `src/main.nr` with your favorite text editor to:
    ...
    ```rust
    fn main(x: pub u32, y: u32) {
        assert(x % y == 0);
    }
    ```

    This program takes a public `x` and a secret `y`, then checks whether `x` is divisible by `y`.
...
You've now successfully written and proven a Noir program! Feel free to play around, for example, if you change `y` to `6` in `Prover.toml`, you'll get a prove error.
