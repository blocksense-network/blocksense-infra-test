...
impl TriggerExecutor for OracleTrigger {
  ...
  async fn new(engine: spin_trigger::TriggerAppEngine<Self>) -> anyhow::Result<Self> {
    let sequencer = engine
      ...
      .sequencer.expect("Sequencer URL is not provided");
  ...
  async fn run(self, config: Self::RunConfig) -> anyhow::Result<()> {
    ...
    let mut loops: Vec<_> = self.queue_components
      .into_values()
...
impl OracleTrigger {
  ...
  async fn process_payload(
    ...
    while let Some((feed_id, payload)) = rx.recv().await {
      let payload_json = DataFeedPayload {
        ...
        result: FeedResult::Result { result: FeedType::Numerical(payload.body.unwrap() as f64) },
      };

      let client = reqwest::Client::new();
      match client.post(sequencer.clone()).json(&payload_json).send().await {
        Ok(res) => {
  ...
  async fn execute_wasm(
    ...
    match instance.call_handle_oracle_request(&mut store, &component.oracle_settings).await {
      Ok(Ok(payload)) => {
...
pub fn current_unix_time() -> u64 {
  SystemTime::now().duration_since(UNIX_EPOCH).expect("SystemTime before UNIX EPOCH!").as_secs()
}
