...
const getEvents = async (
    ...
    const topics = await Promise.all(Object.keys(topicData).map(
        async (eventName) => (await contract.filters[eventName]().getTopicFilter())[0]
    ))
...
(async () => {
    try {
        ...
        console.log("latest event in the last 10k blocks:", await fetchLatestEvent(proxyAddress))
