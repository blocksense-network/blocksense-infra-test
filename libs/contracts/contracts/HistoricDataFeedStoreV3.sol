...
contract HistoricDataFeedStoreV3 {
  ...
  // keccak256('storage.counters_denominator')
  bytes32 internal constant COUNTERS_DENOMINATOR =
    0x338cbc75579d8a2cc788e6632351f053c22753b62efb97e83c2706d324221ed5;
  ...
  fallback(bytes calldata) external returns (bytes memory) {
    ...
    assembly {
      ...
      if and(selector, CONTRACT_MANAGEMENT_SELECTOR_GET_FEED_LATEST) {
        let key := and(
          selector,
          not(CONTRACT_MANAGEMENT_SELECTOR_GET_FEED_LATEST)
        )
      ...
      if and(selector, CONTRACT_MANAGEMENT_SELECTOR_GET_LATEST_COUNTER) {
        mstore(
          0x00,
          sload(
            and(selector, not(CONTRACT_MANAGEMENT_SELECTOR_GET_LATEST_COUNTER))
          )
        )
