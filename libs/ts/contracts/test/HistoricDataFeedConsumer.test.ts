...
let contractWrappers: HistoricDataFeedStoreConsumerBaseWrapper<HistoricDataFeedStore>[] =
  [];
let genericContractWrappers: HistoricDataFeedStoreConsumerBaseWrapper<GenericHistoricDataFeedStore>[] =
  [];
  ...
  for (let i = 1; i <= 100; i *= 10) {
    ...
    it(`Should fetch ${i} feeds at 2 out of 3 counters in a single transaction`, async function () {
      ...
      for (const [index, wrapper] of [
        ...
        let setReceipt;
        if (wrapper instanceof HistoricDataFeedStoreGenericConsumerV1Wrapper) {
